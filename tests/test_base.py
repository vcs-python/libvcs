...
from libvcs.base import VCSRepo
...
def test_repr():
    ...
    assert 'Git' in str_repo
    assert 'hello' in str_repo
    assert '<Git hello>' == str_repo


def test_repr_base():
    repo = VCSRepo(url='file://path/to/myrepo', repo_dir='/hello/')

    str_repo = text_type(repo)
    assert 'Repo' in str_repo
    assert 'hello' in str_repo
    assert '<VCSRepo hello>' == str_repo
...
def test_check_destination_creates_parent_if_not_exist(tmpdir):
    ...
    repo = VCSRepo(url='file://path/to/myrepo', repo_dir=str(repo_dir))
...
def test_get_url_and_revision_from_pip_url():
    url, rev = VCSRepo.get_url_and_revision_from_pip_url(
