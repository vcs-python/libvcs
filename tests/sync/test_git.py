...
def test_remote_is_fast_for_repos_with_many_refs(
    ...
    remote = git_repo.get_remote("origin")
    elapsed = time.monotonic() - started

    assert remote is not None
    assert remote.fetch_url, "fetch URL must be populated"
    assert elapsed < 5.0, f"get_remote() too slow: {elapsed:.2f}s with 500 fake refs"
...
def test_remote_swallows_libvcs_exception(
    ...
    """``GitSync.get_remote`` returns ``None`` when ``git remote -v`` fails.
    ...
    assert git_repo.get_remote("origin") is None
